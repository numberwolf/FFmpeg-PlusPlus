...
error:
            av_log(s, AV_LOG_ERROR,
                   ...
                   "choose from (96000, 44100, 22050, 11025)\n", par->sample_rate);
            return AVERROR(EINVAL);
        }
    }
    ...
    case AV_CODEC_ID_OPUS:
        flags |= FLV_CODECID_OPUS    | FLV_SAMPLESSIZE_16BIT;
        break;
...
static void flv_write_codec_header(AVFormatContext* s, AVCodecParameters* par, int64_t ts) {
    ...
    if (par->codec_id == AV_CODEC_ID_AAC 
        || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_AV1
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4) {
        ...
        if (par->extradata_size > 0 && *(uint8_t*)par->extradata != 1) {
            //if ((ret = ff_av1_parse_seq_header(pkt->data, &data, &size)) < 0) {
